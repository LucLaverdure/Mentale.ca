...
function get_dirs() {
	...
	foreach($dirs as $dirX) {
		//str_replace(WIZBUI_PLUGIN_URL,WIZBUI_PLUGIN_PATH,$_POST['dir']);
		$dir_array[] = "http://".str_replace(WIZBUI_PLUGIN_PATH."cache/", '', $dirX."\n");
		var_dump("http://".str_replace(WIZBUI_PLUGIN_PATH."cache/", '', $dirX."\n"));
		die();
		//$dir_array[] = str_replace(WIZBUI_PLUGIN_PATH."cache/", WIZBUI_PLUGIN_URL."cache/", $dirX."\n");
	}
	return $dir_array;
}
...
function parseAfterOp($html, $op, $opeq) {
	...
	switch ($opeq) {
		...
		case "MD5 Hash":
			...
			break;
		case "Base64 Encode":
			$html = base64_encode(trim($html));
			break;
		case "Base64 Decode":
			$html = base64_decode(trim($html));
			break;
		case "SHA1 Hash":
			$html = sha1(trim($html));
			break;
...
function update_item($the_query, $build_fields, $jc_val) {
	...
	while ( $the_query->have_posts() ) : $the_query->the_post();
		...
		foreach ($build_fields as $kname => $val) {
			if ($kname=="mfn-page-items") { $meta[$kname] = $val; }
			else {
